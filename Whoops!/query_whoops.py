...
class WHOOPSQuerySpecificSGGenerator:
    def __init__(self):
        self.processor = None
        self.model = None
        self.load_model()
    ...
    def _extract_and_clean_object_list(self, raw_object_list):
        """Extract and clean object list from WHOOPS format"""
    ...
    def _validate_result_object(self, obj):
        """Validate that object has required WHOOPS fields"""
        return (isinstance(obj, dict) and 
                'image_id' in obj and 
                'qa_pairs' in obj and
                obj.get('image_id') is not None and 
                obj.get('qa_pairs'))
    
    def generate_query_specific_sg_for_question(self, image, question, object_list, global_scene_graph=""):
        """Generate query-specific scene graph for a single question"""
        try:
...
{{"""
    ...
    def load_and_clean_whoops_results(self, filepath="results_whoops.json"):
        """Load and clean WHOOPS results"""
        print(f"Loading WHOOPS results from {filepath}...")
    ...
    def load_whoops_images(self):
        """Load WHOOPS dataset images with optimized lookup"""
        ...
        try:
            ...
            for split in ['train', 'test']:
                if split in dataset:
                    ...
                    print(f"Loaded WHOOPS {split} set with {len(split_data)} samples")
                    all_data.extend(list(split_data))
            
            print(f"Total WHOOPS samples: {len(all_data)}")
        ...
        for i, sample in enumerate(tqdm(all_data, desc="Indexing images")):
            try:
                ...
                if image and hasattr(image, 'convert'):
                    image_lookup[str(image_id)] = image.convert('RGB')
    ...
    def process_query_specific_sg(self, results_file="results_whoops.json", max_samples=None, debug=False):
        """Generate query-specific scene graphs from WHOOPS results"""
        
        results = self.load_and_clean_whoops_results(results_file)
        
        if not results:
            print("No valid results found! Please check your results file.")
            return []
        
        print(f"Found {len(results)} WHOOPS samples to process")
        
        if max_samples:
            results = results[:max_samples]
            print(f"Limited to {len(results)} samples")
        
        image_lookup = self.load_whoops_images()
        
        if not image_lookup:
            print("No images loaded from WHOOPS dataset!")
            return []
        ...
        print("Generating query-specific scene graphs for WHOOPS...")
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_dir = f"whoops_query_sg_results_{timestamp}"
        os.makedirs(results_dir, exist_ok=True)
        
        for i, result in enumerate(tqdm(results, desc="Processing WHOOPS samples")):
            try:
                image_id = str(result['image_id'])
                qa_pairs = result.get('qa_pairs', [])
                designer_explanation = result.get('designer_explanation', '')
                selected_caption = result.get('selected_caption', '')
                commonsense_category = result.get('commonsense_category', 'unknown')
                ...
                if image_id not in image_lookup:
                    print(f"Image with ID {image_id} not found in dataset")
                    missing_images += 1
                    continue
                
                image = image_lookup[image_id]
                
                # Generate query-specific scene graphs for each question
                question_scene_graphs = []
                ...
                for qa_pair in qa_pairs:
                    if isinstance(qa_pair, dict):
                        ...
                        question = str(qa_pair) if qa_pair else ''
                        answer = ''
                    
                    if not question:
                        continue
                    
                    query_sg = self.generate_query_specific_sg_for_question(
                    ...
                    question_scene_graphs.append(question_sg_data)
                    ...
                    try:
                        ...
                        if 'objects' in parsed_sg or 'relationships' in parsed_sg:
                            ...
                            if debug and i < 5:
                                print(f"Sample {i} - Question: {question[:50]}... - Generated SG: {query_sg[:100]}...")
                    except:
                        print(f"Warning: Invalid JSON generated for sample {i}, question: {question[:50]}...")
                
                # Store result with all question-specific scene graphs
                sg_result = {
                    'image_id': image_id,
                    'original_dataset_index': result.get('original_dataset_index', i),
                    ...
                    'question_scene_graphs': question_scene_graphs,
                    'num_questions': len(question_scene_graphs),
                ...
                if (i + 1) % 50 == 0:
                    intermediate_file = os.path.join(results_dir, f'whoops_sg_intermediate_{i+1}.json')
        ...
        final_results_file = os.path.join(results_dir, 'whoops_sg_final.json')
        with open(final_results_file, 'w') as f:
            json.dump(query_sg_results, f, indent=2)
        
        with open('whoops_query_sg.json', 'w') as f:
            json.dump(query_sg_results, f, indent=2)
        ...
        summary = {
            ...
            'success_rate': successful_generations / sum(r['num_questions'] for r in query_sg_results) if query_sg_results else 0,
            'processing_rate': len(query_sg_results) / len(results) if results else 0,
            'results_directory': results_dir,
            'total_questions_processed': sum(r['num_questions'] for r in query_sg_results),
            'categories_processed': list(set([r.get('commonsense_category', 'unknown') for r in results]))
        }
        
        summary_file = os.path.join(results_dir, 'whoops_processing_summary.json')
        ...
        print(f"Also saved to: whoops_query_sg.json")
...
def main():
    """Main function for WHOOPS query-specific scene graph generation"""
    print("WHOOPS Query-Specific Scene Graph Generation")
    print("=" * 60)
    
    results_file = "results_whoops.json"
    max_samples = None
    debug_mode = True
    ...
    try:
        generator = WHOOPSQuerySpecificSGGenerator()
        ...
        if results:
            ...
            for key, value in sample.items():
                ...
                elif isinstance(value, list) and key == 'question_scene_graphs':
                    print(f"  {key}: {len(value)} question scene graphs")
                    if value:
                        print(f"    First question: {value[0].get('question', '')[:50]}...")
                else:
                    print(f"  {key}: {value}")
            ...
            for result in query_sg_results:
                for qsg in result.get('question_scene_graphs', []):
                    try:
                        sg = json.loads(qsg['query_specific_scene_graph'])
                        if 'error' in sg:
                            error_count += 1
            ...
            total_questions = valid_count + empty_count + error_count
            print(f"\nScene Graph Quality (across all questions):")
            ...
            print(f"  Success rate: {valid_count/total_questions*100:.1f}%" if total_questions > 0 else "  Success rate: 0%")
            
            if empty_count > total_questions * 0.8:
